...
| `audio-to-srt-transcription` Voxtral | `internal/provider.VoxtralProvider` | `provider_test.go` | Direct Mistral HTTP implementation and cue conversion covered |
...
| `audio-to-srt-transcription` Vertex Gemini | `internal/provider.VertexGeminiProvider` | `provider_test.go` | Native client boundary and response parsing covered; no Python fallback |
| `audio-to-srt-transcription` Sherpa Parakeet | `internal/provider.SherpaParakeetProvider` | `provider_test.go` | Cache, audio prep, runtime candidate, token/segment conversion covered; no Python fallback |
